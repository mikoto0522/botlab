...
test('shared order-book preview uses fallback bids for exits when visible bids are missing', () => {
  const snapshot = createSnapshot({
    upOrderBook: {
      bids: [],
      asks: [{ price: 0.53, size: 50 }],
  ...
  assert.ok(preview);
  assert.equal(preview.bookVisible, false);
  assert.equal(preview.quotedPrice, 0.52);
  assert.equal(preview.shares, 2);
});
