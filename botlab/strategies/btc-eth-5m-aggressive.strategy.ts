...
export const strategy: BotlabStrategyDefinition<BtcEth5mParams> = {
  id: 'btc-eth-5m-aggressive',
  name: 'BTC / ETH 5m Binary Pattern Aggressive',
  description: 'An aggressive BTC/ETH 5m prediction-market strategy that allows a wider BTC fade zone and a slightly wider ETH stretch zone.',
  defaults: {
    lookbackCandles: 5,
    entryScore: 5,
  ...
  evaluate(context, params) {
    ...
    if (context.position.side !== 'flat') {
      return {
        ...
        tags: ['btc-eth-5m-aggressive', 'idle'],
      };
    }
    ...
    if (binaryMarket) {
      ...
      if (binaryProfile.mode === 'follow' && rising >= streak) {
        return {
          ...
          tags: ['btc-eth-5m-aggressive', 'entry', 'up', 'binary-follow'],
        };
      }
      ...
      if (binaryProfile.mode === 'follow' && falling >= streak) {
        return {
          ...
          tags: ['btc-eth-5m-aggressive', 'entry', 'down', 'binary-follow'],
        };
      }
      ...
      if (binaryProfile.mode === 'fade' && rising >= streak) {
        return {
          ...
          tags: ['btc-eth-5m-aggressive', 'entry', 'down', 'binary-fade'],
        };
      }
      ...
      if (binaryProfile.mode === 'fade' && falling >= streak) {
        return {
          ...
          tags: ['btc-eth-5m-aggressive', 'entry', 'up', 'binary-fade'],
        };
      }
    ...
    if (score >= params.entryScore) {
      return {
        ...
        tags: ['btc-eth-5m-aggressive', 'entry', 'up'],
      };
    }
    ...
    if (score <= -params.entryScore) {
      return {
        ...
        tags: ['btc-eth-5m-aggressive', 'entry', 'down'],
      };
    }
