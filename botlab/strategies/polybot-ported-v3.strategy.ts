...
interface PolybotPortedV3Params extends Record<string, unknown> {
...
function timingScoreAdjustment(timing: EntryTimingProfile, params: PolybotPortedV3Params): number {
  ...
  if (
    ...
    candidates.push({
      ...
      reason: timing.inPreferredWindow
        ...
        : `${summary.asset} started carrying early enough for a smaller starter entry before the move gets crowded`,
      tags: [
        'polybot-ported-v3',
        'continuation',
        'early-entry',
...
function reversionCandidate(
  ...
  return {
    ...
    tags: ['polybot-ported-v3', 'reversion', 'confirmed-entry'],
  };
}
...
function relativeValueCandidate(
  ...
  const score = 0.9 + Math.min(Math.abs(gap), 3) * 0.4 + timingScoreAdjustment(timing, params);
  ...
  return {
    ...
    tags: ['polybot-ported-v3', 'relative-value', 'confirmed-entry'],
  };
}
...
function evaluatePortedStrategyV3(
  context: BotlabStrategyContext,
  params: PolybotPortedV3Params,
): BotlabStrategyDecision {
  ...
  if (peerMarket) {
    ...
    if (peerSummary) {
      const relative = relativeValueCandidate(summary, peerSummary, params, timing);
      if (relative) {
  ...
  if (entryPrice < params.minEntryPrice || entryPrice > params.maxEntryPrice) {
    return {
      ...
      tags: ['polybot-ported-v3', 'idle', 'price-filter'],
    };
  }
  ...
  if (best.tier === 'early') {
    if (timing.lateWindow) {
      return {
        ...
        tags: ['polybot-ported-v3', 'idle', 'late-window'],
      };
    }
    ...
    if (best.score < earlyScoreFloor) {
      return {
        ...
        tags: ['polybot-ported-v3', 'idle', 'weak'],
      };
    }
...
export const strategy: BotlabStrategyDefinition<PolybotPortedV3Params> = {
  id: 'polybot-ported-v3',
  name: 'Polybot Ported V3',
  description: 'Pushes the polybot port toward a preferred 90-to-120 second entry window, relaxes early starters there, and stops marginal late chases unless the confirmation is clearly strong.',
  defaults: {
    lookbackCandles: 6,
    minimumVolume: 750,
  ...
  evaluate: evaluatePortedStrategyV3,
};
