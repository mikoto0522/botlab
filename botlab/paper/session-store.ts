...
import {
  ...
  type PaperSessionPaths,
  type PaperSessionPredictionSide,
  type PaperSessionPosition,
  type PaperSessionRollingHistory,
  type PaperSessionState,
  type PaperSessionSummary,
  resolvePaperSessionPaths,
} from './types.js';
...
function createSessionLoadError(sessionName: string, statePath: string, reason: string): Error {
  return new Error(`Failed to load paper session "${sessionName}" from ${statePath}: ${reason}`);
}

function createEmptyHistory(): PaperSessionRollingHistory {
  return {
    maxLength: PAPER_SESSION_HISTORY_LIMIT,
    points: [],
  };
}

function createDefaultHistoryMap(): PaperSessionHistoryMap {
  return {
...
function inferPredictionSide(side: PaperSessionPosition['side']): PaperSessionPredictionSide | null {
  if (side === 'long') {
    return 'up';
  }
  if (side === 'short') {
...
function readPosition(value: unknown, sessionName: string, statePath: string, asset: string): PaperSessionPosition {
  if (!isRecord(value)) {
  ...
  return {
    ...
    side: value.side,
    predictionSide: (value.predictionSide as PaperSessionPredictionSide | null | undefined) ?? inferPredictionSide(value.side as PaperSessionPosition['side']),
    size: value.size,
...
function readHistory(value: unknown, sessionName: string, statePath: string, asset: string): PaperSessionRollingHistory {
  if (!isRecord(value)) {
...
function readHistoryMap(value: unknown, sessionName: string, statePath: string): PaperSessionHistoryMap {
  if (!isRecord(value)) {
...
function normalizeState(raw: unknown, sessionName: string, sessionSlug: string, statePath: string): PaperSessionState {
  if (!isRecord(raw)) {
  ...
  const positions: Partial<Record<'BTC' | 'ETH', PaperSessionPosition>> = {};
...
function toSummary(state: PaperSessionState): PaperSessionSummary {
  return {
...
function shouldPersistPaperSessionEvent(event: PaperSessionEvent): boolean {
  if (event.type === 'paper-strategy-decision') {
    return event.action !== 'hold' && event.action !== 'flat';
  }

  if (event.type === 'paper-cycle-complete') {
...
export function createEmptyPaperSessionState(
  sessionName: string,
  options: { startingCash?: number; now?: string } = {},
): PaperSessionState {
  const now = options.now ?? new Date().toISOString();
  const startingCash = isFiniteNumber(options.startingCash) ? options.startingCash : PAPER_SESSION_STARTING_CASH;
  const { sessionSlug } = resolvePaperSessionPaths(process.cwd(), sessionName);
...
export function loadPaperSessionState(
  sessionName: string,
  cwd = process.cwd(),
  options: { startingCash?: number } = {},
): PaperSessionState {
  const paths = resolvePaperSessionPaths(cwd, sessionName);

  if (!fs.existsSync(paths.statePath)) {
    return createEmptyPaperSessionState(sessionName, { startingCash: options.startingCash });
  }
...
export function savePaperSessionState(state: PaperSessionState, cwd = process.cwd()): PaperSessionPaths {
  const paths = resolvePaperSessionPaths(cwd, state.sessionName);
...
export function appendPaperSessionEvent(sessionName: string, event: PaperSessionEvent, cwd = process.cwd()): PaperSessionPaths {
  const paths = resolvePaperSessionPaths(cwd, sessionName);

  if (!shouldPersistPaperSessionEvent(event)) {
    return paths;
  }
...
export function resumePaperSessionState(
  sessionName: string,
  cwd = process.cwd(),
  options: { startingCash?: number } = {},
): PaperSessionState {
  return loadPaperSessionState(sessionName, cwd, options);
}
